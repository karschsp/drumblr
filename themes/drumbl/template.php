 ...
 */
function drumbl_preprocess_page(&$vars) {
 ...
 */
function drumbl_preprocess_node(&$vars) {
  $node = $vars['node'];
  $vars['submitted'] = '<div class="byline">' . ucwords($node->type) . ' Posted on: </div><div class="date">' . date('M d, Y', $node->created) . '</div>';
  if ($node->type == substr($node->title, 0, strlen($node->type))) {
    $vars['title'] = '';
  }
}